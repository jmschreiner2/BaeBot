...
module.exports = {
    getPictureUrl: function (tag) {
        let data = request('GET', `https://rule34.xxx/index.php?page=dapi&s=post&q=index&tags=${tag}&limit=1`);
        let obj = null;
        ...
        data = request('GET', `http://rule34.xxx/index.php?page=dapi&s=post&q=index&tags=${tag}&pid=${page}`);
        xml.parseString(data.getBody('utf8'), (err, result) =>{
            obj = result;
        });
